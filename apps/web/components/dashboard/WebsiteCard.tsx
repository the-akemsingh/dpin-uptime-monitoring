...
export function WebsiteCard({ id, url, statusBuckets, onDelete }: WebsiteCardProps) {
  ...
  return (
    <div className="flex min-w-7xl gap-10">
      <div className="group mt-5 rounded-xl border border-zinc-800 bg-[#0a0a0a] p-4 shadow-sm transition-all duration-200 hover:shadow-md hover:border-zinc-700 flex flex-col gap-4">
        <div className="flex flex-col flex-1">
          ...
          <div className="flex items-center gap-1.5 px-1 py-2">
            {statusBuckets.map((status, index) => (
              ...
              >
                <div className="pointer-events-none absolute -top-8 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded bg-zinc-800 px-2 py-1 text-xs px-2 shadow-lg ring-1 ring-white/10 opacity-0 transition-opacity group-hover/tick:opacity-100">
                  {status === "good" && <span className="text-emerald-400 font-medium">Good</span>}
                  {status === "bad" && <span className="text-red-400 font-medium">Bad</span>}
                  {status === "no-info" && <span className="text-zinc-400 font-medium">Unknown</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {latencyData.length > 0 && (
        <div className="mt-5 rounded-xl border border-zinc-800 bg-[#0a0a0a] p-4 shadow-sm transition-all duration-200 hover:shadow-md hover:border-zinc-700 flex flex-col gap-4">
          <div className="w-full">
            <div className="grid grid-cols-3 items-center gap-10 border-b border-zinc-800/80 px-2 pb-3 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
            ...
            <div className="flex flex-col divide-y divide-zinc-800/40">
              {latencyData.map((reg) => (
                <div key={reg.region} className="grid grid-cols-3 items-center gap-2 px-2 py-3 text-sm">
                  <span className="truncate text-left text-xs font-mono text-zinc-300">{reg.region.toLowerCase()}</span>
                  <span className="text-center text-xs font-mono text-zinc-300">{reg.latency}ms</span>
                  <span
                    className={`text-right text-xs font-mono font-semibold ${reg.status.toUpperCase() === "UP" ? "text-green-500" : "text-red-500"}`}
                  >
